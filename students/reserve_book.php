                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
						...
						<div class="row">
							<form method="post">
								<div class="col-xs-4">
									<input style="margin-bottom:10px; margin-left:-9px;" type="datetime-local" name="pickup_date" class="form-control"/>
								</div>
								<div class="col-xs-4">
									...
									<select name="barcode" style="margin-bottom:10px; margin-left:-9px;" class="form-control" required>
										<option value="">--Select Book--</option>
										<?php 
											$result= mysqli_query($con,"select * from book INNER JOIN category ON category.category_id = book.category_id where status NOT IN ('Damaged', 'Lost') AND archive='0' AND book_copies>=1") or die (mysqli_error());
											while ($row= mysqli_fetch_array ($result) ){
										?>
										<option value="<?php echo $row['book_barcode']?>"><?php echo '('.$row['book_copies'].') '.$row['book_title']." (".$row['classname'].")"?></option>
										<?php } ?>
									</select>
								</div>
								<div class='col-xs-4'>
									<input type="submit" style="margin-bottom:10px; margin-left:-9px;" name="submit" class="btn btn-primary btn-sm">
								</div>
							</form>
							<table class="table table-bordered" id="reload">
								...
								<td>
										...
										<button name="borrow" id="savenBtn" class="btn btn-info"><i class="fa fa-check"></i> Reserve</button>
